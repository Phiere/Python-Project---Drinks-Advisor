...
# Etapes :
# - Renseigner les chemins de départ et d'arrivée des différentes Data_Frame (voir ....)
# - Traitement 1 : remplacer les colonnes doubles : (ingrédient1, ingrédient2,...) par une colonne contenant la liste des éléments ([ingrédient1, ingrédient2])
# - Traitement 2 : normalise les éléments dans une même colonne : (Vodka , vodka --> Vodka).
# - Traitement 3 : rajouter à chaque data_base les colonnes PersonalRating, Comment, Favori
...
def trouver_indices(df_columns_names):
        """Sort les indices des colonnes en double, i.e des colonnes avec le même nom (une fois leur caractères non str enlevés)
...
def creation_list_colonnes_doubles(data_base):
        """Crée le clône du data_frame en entreé en changeant les colonnes jumelles par une colonne avec une liste de leurs éléments"""
...
def creation_unique_elements_data_frame(data_base):
        """Cette fonction crée un clône de la database en entrée en supprimant tous les éléments en doublon pour ne garder que les éléments
        ...
        #Création des colonnes sans répétitions
...
def suppression_unnamed(data_base):
     """Suppression des colonnes Unnamed qui se rajoutent parfois lors de la manipulation de data_base"""
...
def delete_unnecessary_column(data_frame,unnecessary_column):
     """Supprime les colonnes des databses inutiles poour l'application"""
...
def raw_data_traitement(raw_data_frame_path,listed_data_frame_path,uniques_element_data_frame_path,unnecessary_column,new_column_names):
        """Applique tous les traitements décrits en introduction aux data_base utilisées dans l'application
